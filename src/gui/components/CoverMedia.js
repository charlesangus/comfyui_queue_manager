...
export const CoverMedia = memo(function CoverMedia({item, force}) {
  ...
  function getCoverFile(item) {
    ...
    Object.keys(item.outputs).find(nodeID => {
      ...
      const isImage = outputs.images && outputs.images.length > 0 && (!outputs.animated || outputs.animated[0] !== true);
      const isVideo = (outputs.images && outputs.images.length > 0 && outputs.animated && outputs.animated[0] === true) || (outputs.gifs && outputs.gifs.length > 0);

      // console.log("Outputs", outputs, "isImage:", isImage, "isVideo:", isVideo);

      if (isImage) {
        console.log("Image outputs", outputs, "ShowImages:", appStatus.options.ShowImages);
        ...
        if (appStatus.options.ShowVideos || force) {
          console.log("Video outputs", outputs);
          video = (outputs.gifs && outputs.gifs.length > 0) ? outputs.gifs[0] : outputs.images[0];
          _nodeID = nodeID;
